...
<template>
  <div class="offline-detail-page">
    ...
    <!-- 📱 移动形态：功能按钮横向可滚动（内容顶部，不换行不撑满；桌面端隐藏） -->
...
/* 📱 移动形态（html[data-layout='mobile']）：详情页顶部重设计（方案B改良，2026-08-06）
   ...
   - 功能按钮 → 内容顶部横向可滚动按钮条（overflow-x:auto，图标+文字，不换行不撑满）
   - 原顶部操作栏整行隐藏（返回钮/功能钮已由上述两元素替代；同时覆盖 App.vue 全局 .top-bar 的 56px/背景）
...
:global(html[data-layout='mobile'] .offline-detail-page .top-bar) {
  display: none;
}

:global(html[data-layout='mobile'] .offline-detail-page .detail-fab-back) {
  display: flex;
  position: fixed;
...
:global(html[data-layout='mobile'] .offline-detail-page .detail-actions-bar) {
  display: flex;
  align-items: center;
  gap: 8px;
  overflow-x: auto;
...
:global(html[data-layout='mobile'] .offline-detail-page .detail-actions-bar .add-reading-btn),
:global(html[data-layout='mobile'] .offline-detail-page .detail-actions-bar .read-btn),
:global(html[data-layout='mobile'] .offline-detail-page .detail-actions-bar .delete-btn) {
  flex-shrink: 0;
  white-space: nowrap;
}

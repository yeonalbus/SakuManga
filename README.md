...
v2.0.0 为里程碑大版本：项目正式更名为 **SakuManga**（包名 / exe / 界面标题 / 文档全量同步），并一次性汇总 v1.4.0 以来的画质升级、搜刮书签、查重增强、阅读器升级等新能力，继续推进稳定性与体验主线。
...
- **在线浏览**：首页 / 热门 / 订阅 / 收藏 / 历史 / 排行榜（昨日 · 本月 · 本年 · All-Time 四种类型，评分对齐 E 站真实值）/ 随机抽卡 / **搜刮书签**（任意位置存为书签，一键恢复筛选并锚定定位卡片）
...
- **在线搜索**：f_search 自动修正为 E 站标准语法（多词 `t:... s:...` 完整命中）、Tag 联想/点击统一按线上格式生成、**点击联想保持输入焦点可连续输入多 tag**、**搜索配置持久化（搜索跳转后筛选不再重置）**、联想按标签热度协同排序、搜索新标签打开
...
- **设置中心**：分组收编为「账户 / E 站连接 / 阅读体验 / 下载与离线 / 标签管理 / 日志 / 关于」，每项设置均真实生效；**账号设置显示 pass_hash 并支持一键刷新凭证**；内置日志系统（更新 / 维护 / 下载 / 错误四类日志）
- **性能与安全**：**E 站请求自适应限流（连续成功提速至 500ms、429/503 自动退避，全量核对耗时约减半）**；调试工具 E 站凭据改环境变量注入，移除硬编码凭据
...
| 分组       | 栏目       | 说明                                                         |
| ---------- | ---------- | ------------------------------------------------------------ |
| 账户       | 账户       | E 站账号与 Cookie 配置（含 pass_hash 展示与一键刷新凭证）、成员管理、服务器信息 |

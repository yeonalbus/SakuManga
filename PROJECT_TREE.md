...
└── views/
    ├── ComicReader.vue             # 漫画阅读器（RTL/左右键/雪碧图）
...
| 改阅读器（翻页/快捷键/RTL） | `src/views/ComicReader.vue`                                                                                                |
...
| 改在线画廊接口             | `backend/internal/handlers/online.go`、`services/eh_gallery.go`、`eh_detail.go`          |

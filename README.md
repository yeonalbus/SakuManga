...
- **在线浏览**：首页 / 热门 / 订阅 / 收藏 / 历史 / 排行榜（昨日 · 本月 · 本年 · All-Time 四种类型，评分对齐 E 站真实值）/ 随机抽卡
- **详情体验**：宽屏点卡自动开小详情、本地优先加载（已下载画廊自动走本地页图 + 一键切回在线）、默认收藏夹、取消收藏 web 弹窗、详情新标签导航 + 智能返回（恢复来源列表位置）
- **阅读恢复**：进度按账号同步后端；历史入口始终续看、非历史入口受「从上次位置开始」偏好控制；在线 token 缺失自动解析兜底；离线历史按 gid 合并去重（更新替换换 id 后不再重复显示同一本子）
- **下载系统**：画廊 + 归档多线程并发、下载优先级 + 抢占式调度、归档并发数控制、断点续传（.part / .bits）、zip 校验失败自动重建会话重下、下载完成自动消除更新标记、已下载/下载中画廊入口拦截
- **本地阅读**：离线书架（自定义排序 / 置顶 / 全部书架检索浮层 / 多选批量加入）/ 历史 / 排行榜 / 更新检测（周扫描 + 365 天老化）/ 维护查重（双列对比、跨文件夹版本识别、失效画廊标记、名称级疑似重复分组与忽略清单、进入页面不再自动扫描）/ 阅读进度续看
- **在线搜索**：f_search 自动修正为 E 站标准语法（多词 `t:... s:...` 完整命中）、Tag 联想/点击统一按线上格式生成、联想按标签热度协同排序、搜索新标签打开
...
- **阅读器**：颜色跟随主题（深色/浅色）、手柄翻页与确认/取消按键、页面加载失败错误层（重试/返回）、离线 404 自动纠正为在线模式（无 token 也兜底解析）

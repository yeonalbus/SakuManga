...
<<<<<<< HEAD
**v1.3.2**（2026-08）：纯稳定性小更新——根除下载归档反复报错「解压 zip 失败」的重大 bug，归档下载链路不再翻车。
=======
[!NOTE]  
**项目更新状态（Project Status）**  
因学业原因，本项目暂时停止新功能迭代，预计将于 **2026 年底** 恢复更新。  
停更期间仓库保持开放，仅用于记录和整理现有 Bug。  

**v1.3.1**（2026-08）：在线搜索系统全面升级——f_search 标准语法、热度协同排序联想与多词完整命中，搜索即所得。
>>>>>>> 1bdb6c8eba259195b1fa98ad4922eccdb7d78261
...
- **阅读恢复**：进度按账号同步后端；历史入口始终续看、非历史入口受「从上次位置开始」偏好控制；在线 token 缺失自动解析兜底
- **下载系统**：画廊 + 归档多线程并发、下载优先级 + 抢占式调度、归档并发数控制、断点续传（.part / .bits）、zip 校验失败自动重建会话重下、下载完成自动消除更新标记、已下载/下载中画廊入口拦截
- **本地阅读**：离线书架 / 历史 / 排行榜 / 更新检测（周扫描 + 365 天老化）/ 维护查重（双列对比、跨文件夹版本识别、失效画廊标记）/ 阅读进度续看
- **在线搜索**：f_search 自动修正为 E 站标准语法（多词 `t:... s:...` 完整命中）、Tag 联想/点击按在线 / 离线模式生成标准语法、联想按标签热度协同排序、搜索新标签打开
...
v1.3.2 为纯稳定性修复版本，聚焦下载归档「解压 zip 失败」的根因收敛。后续将延续稳定性与性能主线：持续收敛下载稳定性与资源占用、完善跨设备进度同步、补充离线端更丰富的检索与维护能力，并探索搜索结果的进一步精准化。
